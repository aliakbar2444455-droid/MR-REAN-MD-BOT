...
async function validateSession(kingbadboiNumber) {
    ...
    try {
        ...
        if (!creds.me || !creds.me.id) {
            console.log(chalk.yellow(`⚠️ Invalid session for ${kingbadboiNumber}; preserving files (no auto-unpair).`));
            return false;
        }
        return true;
    } catch (e) {
        console.log(chalk.red(`❌ Corrupt session for ${kingbadboiNumber}: ${e.message}; preserving files (no auto-unpair).`));
        return false;
    }
}
...
function forceCleanupSession(kingbadboiNumber) {
    ...
    try {
        // Never delete a user's session automatically. A real logout or a
        // transient connection failure must not silently become an /unpair.
        console.log(chalk.yellow(`⚠️ Session preserved: ${kingbadboiNumber} (automatic cleanup disabled)`));
        ...
        console.log(chalk.red(`❌ Error preserving ${kingbadboiNumber}: ${e.message}`));
        return false;
    }
}
    ...
    fs.readdirSync(sessionDir).forEach(folder => {
        ...
        if (fs.lstatSync(folderPath).isDirectory()) {
            ...
            if (tracker && tracker.disconnected) {
                console.log(chalk.gray(`ℹ️ Preserving disconnected session: ${folder}`));
                return;
            }
            ...
            try {
                ...
                if (stats.mtimeMs < oneDayAgo) {
                    console.log(chalk.gray(`ℹ️ Preserving old session: ${folder}`));
                }
            } catch (e) {
...
async function startpairing(kingbadboiNumber) {
    ...
    if (pairingCode && !state.creds.registered) {
        ...
        setTimeout(async () => {
            try {
                let code = await bad.requestPairingCode(phoneNumber, 'MRREANMD');
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            if (reason === 405) {
                ...
                console.log(chalk.yellow(`🛡️ Preserving session for ${kingbadboiNumber}; no automatic unpair`));
