...
const START_IMAGE_URL = 'https://i.postimg.cc/1tNTvGGq/file-00000000cf608211b149c59a57de8036.png';
const START_IMAGE_PATH = path.join(__dirname, 'assets', 'menu-image.png');
...
// The bot must be an administrator in every required chat.
const REQUIRED_CHATS = [
  { id: '@MRreanMAINC', label: 'Channel 1', url: 'https://t.me/MRreanMAINC' },
  { id: '@MRREAN_BANINGPROOF', label: 'Channel 2', url: 'https://t.me/MRREAN_BANINGPROOF' },
  { id: '@MRReanCHATZONE', label: 'Group', url: 'https://t.me/MRReanCHATZONE' }
];
...
const checkUserJoinedChannels = async (userId) => {
  for (const chat of REQUIRED_CHATS) {
    try {
      ...
      console.error(`Force-join check failed for ${chat.id}:`, error.message);
      return false;
    }
  }
  return true;
...
const sendChannelsRequiredMessage = async (chatId) => {
  return bot.sendMessage(chatId,
    `🚨 *You must join our official channels before pairing.*`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          ...REQUIRED_CHATS.map(chat => [{ text: `📢 ${chat.label}`, url: chat.url }]),
...
// Keep the image in the ZIP and upload local bytes so Telegram never has to
// fetch a third-party URL. The URL remains as a backup for older deployments.
const sendStartMessage = async (chatId) => {
  const options = {
    ...
    reply_markup: {
      inline_keyboard: [
        [{ text: '👑 Owner', url: 'https://t.me/LORDHARSHK' }]
      ]
    }
  };

  try {
    if (fs2.existsSync(START_IMAGE_PATH)) {
      return bot.sendPhoto(chatId, fs2.readFileSync(START_IMAGE_PATH), options);
    }

    const response = await axios.get(START_IMAGE_URL, {
    ...
    return bot.sendPhoto(chatId, Buffer.from(response.data), options);
  } catch (error) {
    console.error('START IMAGE ERROR:', error.message);
...
bot.onText(/\/start/, async (msg) => {
  ...
  // Do not reveal the private menu until the user has joined every required
  // channel/group. The same check also protects /start payloads such as
  // /start pair, so users cannot bypass force-join through a deep link.
  const allJoined = await checkUserJoinedChannels(userId);
  if (!allJoined) {
    return sendChannelsRequiredMessage(chatId);
  }

  // Fetch the image locally so Telegram does not need to fetch a third-party URL.
